...
Lemma DefEq_equality_substitution : (forall P phi b1 b2,
  DefEq P phi b1 b2 -> forall P1 x psi, 
       ...
       -> psi <= phi
       -> DefEq P1 phi (subst a1 x b1) (subst a2 x b2)). 
Proof. 
  intros.
  subst.
  move: (DefEq_uniq H) => h. destruct_uniq.
  have RE: DefEq P1 phi (a_Pi psi (a_Type star) (close x b1))
                        (a_Pi psi (a_Type star) (close x b2)).
...
Proof.
  ...
  eapply (@DefEq_renaming x). repeat rewrite fv_close. fsetdec.
  repeat rewrite fv_close. fsetdec.
  rewrite open_close.
  rewrite open_close.
  auto.
Qed.
